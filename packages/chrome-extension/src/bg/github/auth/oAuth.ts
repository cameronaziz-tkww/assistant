...
//   const url = `${SERVICE_ENDPOINT}github-oauth?code=${encode(code)}&redirect=${encode(redirect)}`;
...
const display = async (url: string, state: string): Promise<Storage.Jira.OAuthPayload | string> => {
  ...
  // const authResponse = await fetchToken(params.code, domain);
...
const OAuth = async (): Promise<Storage.Jira.OAuthPayload | string> => new Promise((resolve, reject) => {
  ...
  const query = `client_id=${GITHUB_CLIENT_ID}&redirect_uri=${encode(redirectUri)}&scope=${encode(scope)}&state=${state}`;
